...
void read_document(const fs::path& input, const Json& settings, const std::function<void(int, const Image&)>& consume) {
    ...
    if (std::set<std::string>{".doc", ".docx", ".ppt", ".pptx", ".xls", ".xlsx", ".odt", ".ods", ".odp"}.count(ext)) {
        ...
        // LibreOffice sometimes exits zero while failing to produce a document.
        if (!fs::is_regular_file(pdf) || fs::file_size(pdf) == 0)
            throw std::runtime_error("Office conversion produced no PDF (check encryption, IRM, corruption and fonts)");
    } else if (ext != ".pdf") throw std::runtime_error("unsupported input extension: " + ext);
