...
单文件 CLI 的 `execution.workers=8`、`text.instances=1`、`text.max_concurrent_requests=4` 意味着最多 8 个 BOX 在处理，其中至多 4 个 text 后端调用同时在途；其他线程等待。额外槽位对 HTTP 是独立连接客户端，对本地 ACL/ONNX 是额外模型句柄。设置 `text.batch_size>1` 后，每个并发槽位一次处理最多 batch_size 个请求，模型池按 ID 汇集所有文件的 BOX，并在最大等待窗口结束或收齐一批时调用一次原生批量接口。批处理和服务模式另设全局 `box_workers` 池以免按页面数重复创建 BOX 线程；等待模型的 BOX 任务占用该池的线程，跨模型公平性及任务优先级尚未实现。配置和后端支持范围见 [模型池](configuration.md#模型池)。
...
每个页面线程先完成布局，再将该页最多 `min(BOX 数, box_workers)` 个 BOX 工作任务提交至批次共享的固定线程池，并等待结果；`box_workers` 默认为 1，旧配置保持串行行为。所有页面共用最多 `box_workers` 个 BOX 线程，而非 `page_workers × box_workers`。模型 ID 的 `max_concurrent_requests` 限制真实后端在途调用；当多个页面共享一个模型时也复用同一个队列。Reader 的 PDF 渲染、Office 转换可以与推理并行，它们受 `max_active_documents` 单独限制。内存上限来自 `max_queued_pages` 个已入队图像 + 正在渲染的 reader 图像 + 正在处理的页面、BOX 裁剪及模型中间缓存，不是全程固定的精确字节数。
...
`src/server.cpp`（可选 `OMNIOCR_WITH_SERVER=ON`，依赖 libmicrohttpd）提供 `omniocr-server`：HTTP 线程只接收请求、校验输入并落盘，**不在请求线程运行 OCR**。路径或原始二进制提交后生成不可预测的任务 ID、独立输入/输出目录，随即返回 HTTP 202。后台常驻 `document-workers` 线程从等待文件队列中选择较高优先级任务，执行文档转换和逐页渲染；`page-workers` 全局页面线程从已就绪页队列选择较高优先级页。模型实例池只初始化一次并被整个进程复用；全局 `box-workers` 池可让单页多 BOX 并行，默认 1 保持原有行为，不按页面数扩张 BOX 线程。
