...
`page_workers` 默认取 `execution.workers`；`max_active_documents` 默认 2，`max_queued_pages` 默认 2。批处理模式下每页 BOX 串行处理，避免页面和 BOX 双层线程池相乘；NPU/vLLM 模型并发同时受到各自 `max_concurrent_requests`（默认 `instances`）池大小约束。PDF/Office 转换子进程与页面推理可并行，总进程 CPU/内存仍需按文档尺寸配置。一个文件失败时，其他文件继续写结果；整个批次若有文件级失败，CLI 返回 1；仅有 `record` 模式 BOX 失败返回 2；全部成功返回 0。各任务输出目录必须互不重叠。
